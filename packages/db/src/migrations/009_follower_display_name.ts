...
export async function up(db: Kysely<unknown>): Promise<void> {
  await db.schema.alterTable("followers").addColumn("remote_username", "varchar(255)").execute();

  await db.schema.alterTable("followers").addColumn("remote_domain", "varchar(255)").execute();
}
